...
def compare_csv(s_file, t_file, s_key, t_key, s_delimiter, t_delimiter, s_columns_excluded, t_columns_excluded,
    ...
    elif not s_key and not t_key:
        print('No keys specified for both source and target. Proceeding with row comparison.')
    else:
        sys.exit(f'Inconsistent key specifications: Source key is {'empty' if not s_key else 'specified'},'
                 f'while Target key is {'empty' if not t_key else 'specified'}.')
